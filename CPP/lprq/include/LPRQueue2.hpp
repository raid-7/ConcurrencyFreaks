...
class PRQueue2 : public QueueSegmentBase<T, PRQueue2<T, padded_cells, ring_size>> {
private:
    using Base = QueueSegmentBase<T, PRQueue2<T, padded_cells, ring_size>>;
    using Cell = detail::CRQCell<void*, padded_cells>;

    Cell array[ring_size];

    inline uint64_t nodeIndex(uint64_t i) {
        return (i & ~(1ull << 63));
    }

    inline uint64_t setUnsafe(uint64_t i) {
        return (i | (1ull << 63));
    }

    inline uint64_t nodeUnsafe(uint64_t i) {
        return (i & (1ull << 63));
    }

    inline bool isBottom(void* const value) {
        return (reinterpret_cast<uintptr_t>(value) & 1) != 0;
    }

    inline void* threadLocalBottom(const int tid) {
...
public:
    ...
    static std::string className() {
        ...
        return "PRQueue2"s + (padded_cells ? "/ca"s : ""s);
    }
...
using LPRQueue2 = LinkedRingQueue<T, PRQueue2<T, padded_cells, ring_size>>;
